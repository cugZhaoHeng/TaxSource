...
<script type="text/javascript">
    ...
    $(function(){
        ...
        /* 通过session中的taxerId，显示登录用户的姓名，将userName转化为taxerName */
        $.post("getUser.do", {}, function(data){
        	$.each(data, function(index, val) {
        		//在user表中匹配到userName时， 在用user表中的taxerId，这里在取session中的值时，需要加上大括号
        		if("${sessionScope.userName}" == val.userName) {
        			taxerId = val.taxerId;
	    ...
	    $(".personInfo").on("click", function(e){
	    	$.post("getUser.do",{}, function(data) {
	    		$.each(data, function(index, val) {
	    			if(val.taxerId == "${sessionScope.taxerId}") {
	    				taxerId = "${sessionScope.taxerId}"
	    			}
	    		})
	    	},"json")
	    	 openTopWindow({
	             ...
	             height : 500,
	             title : "查看个人信息",
	             url : "manage/lookPersonInfo.jsp?taxerId="+taxerId
	         })
	    });
